...
mod tests {
    ...
    #[test]
    fn test_small_scale_generation() -> OwlResult<()> {
        let (ontology, events) = EPCISDataGenerator::generate_small_scale()?;
        
        assert!(!ontology.classes().is_empty());
        assert!(!events.is_empty());
        assert!(events.len() >= 100 && events.len() <= 500);
        
        println!("Generated {} events for small scale", events.len());
        Ok(())
    }

    #[test]
    fn test_medium_scale_generation() -> OwlResult<()> {
        let (ontology, events) = EPCISDataGenerator::generate_medium_scale()?;
        
        assert!(!ontology.classes().is_empty());
        assert!(!events.is_empty());
        assert!(events.len() >= 1000 && events.len() <= 5000);
        
        println!("Generated {} events for medium scale", events.len());
        Ok(())
    }

    #[test]
    fn test_large_scale_generation() -> OwlResult<()> {
        let (ontology, events) = EPCISDataGenerator::generate_large_scale()?;
        
        assert!(!ontology.classes().is_empty());
        assert!(!events.is_empty());
        assert!(events.len() >= 10000 && events.len() <= 50000);
        
        println!("Generated {} events for large scale", events.len());
        Ok(())
    }

    #[test]
    fn test_event_structure() -> OwlResult<()> {
        let (ontology, events) = EPCISDataGenerator::generate_small_scale()?;
        let event = &events[0];
        
        assert!(!event.event_id.is_empty());
        assert!(matches!(event.event_type, EPCISEventType::ObjectEvent | EPCISEventType::AggregationEvent | EPCISEventType::TransactionEvent | EPCISEventType::TransformationEvent));
        
        // Test OWL2 conversion
        let (event_ontology, iris) = event.to_owl2()?;
        assert!(!event_ontology.classes().is_empty());
