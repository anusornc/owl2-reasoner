...
mod tests {
    ...
    #[test]
    fn test_smallvec_creation() {
        let iris = SmallVecUtils::iris();
        assert!(iris.is_stack_allocated());
        assert_eq!(iris.inline_capacity(), sizes::ENTITIES);
    }

    #[test]
    fn test_smallvec_ext() {
        let vec = vec![1, 2, 3];
        let smallvec = vec.to_optimized_smallvec();
        assert_eq!(smallvec.len(), 3);
        assert!(smallvec.is_stack_allocated());
    }

    #[test]
    fn test_extend_up_to_capacity() {
        let mut smallvec = SmallVec::<[i32; 8]>::new();
        let items = vec![1, 2, 3, 4, 5, 6, 7, 8]; // More than capacity
        ...
        assert_eq!(smallvec.len(), 8);
        assert!(smallvec.is_stack_allocated());
    }

    #[test]
    fn test_smallvec_macro() {
        let iri1 = IRI::new("http://example.org/test1".to_string()).unwrap();
        let iri2 = IRI::new("http://example.org/test2".to_string()).unwrap();

        let mut iris = SmallVec::<[IRI; 4]>::new();
        iris.push(iri1);
        iris.push(iri2);
