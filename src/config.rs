...
mod tests {
    ...
    fn test_config_builder() {
        let config = OwlConfig::builder()
            .cache(CacheConfig::builder().iri_cache_size(2000).build().unwrap())
            .reasoning(ReasoningConfig::default())
            .parser(ParserConfig::default())
            .performance(PerformanceConfig::default())
            .build();
        ...
        assert!(config.is_ok());
        assert_eq!(config.unwrap().cache.iri_cache_size, 2000);
    }

    #[test]
    fn test_profiles() {
        assert!(profiles::development().validate().is_ok());
        assert!(profiles::production().validate().is_ok());
        assert!(profiles::testing().validate().is_ok());
    }

    #[test]
    fn test_invalid_config() {
        let mut cache_config = CacheConfig::default();
        cache_config.iri_cache_size = 0;
