...
mod tests {
    ...
    #[test]
    fn test_memory_aware_allocator_creation() {
        let allocator = MemoryAwareAllocator::new();
    ...
    fn test_successful_allocation() {
        let mut allocator = MemoryAwareAllocator::new();
        
        let result = allocator.allocate("test", 1024, || 42u32);
        assert!(result.is_ok());
        
        let allocation = result.unwrap();
        assert_eq!(allocation.item, 42);
        assert_eq!(allocation.size, 1024);
        assert!(!allocation.was_adapted);
    }
    ...
    fn test_adaptive_sizing() {
        ...
        let result = allocator.allocate("test", 10000, || vec![0u8; 10000]);
        assert!(result.is_ok());
        
        let allocation = result.unwrap();
        assert!(allocation.size <= 10000); // Should be adapted if under pressure
    }
    ...
    fn test_batch_operations() {
        ...
        // Add items to batch
        for i in 0..5 {
            let result = allocator.add_to_batch("test".to_string(), 1024, i + 1);
            assert!(result.is_ok());
        }
        
        // Process batch
        let results = allocator.process_batch(|| vec![0u8; 1024]);
    ...
    fn test_pool_allocation() {
        let config = AllocationStrategyConfig {
            enable_memory_pools: true,
            pool_limit: 10,
            ..Default::default()
        };
        
        let mut allocator = MemoryAwareAllocator::with_config(config);
        
        // First allocation should come from creator
        let result1 = allocator.allocate("test", 1024, || 42u32);
        assert!(result1.is_ok());
        
        // Second allocation might come from pool if it's the same size
        let result2 = allocator.allocate("test", 1024, || 43u32);
        assert!(result2.is_ok());
        
        let stats = allocator.get_allocator_stats();
    ...
    fn test_cleanup() {
        ...
        for i in 0..20 {
            let result = allocator.allocate("test", 1024, || i as u32);
            assert!(result.is_ok());
        }
        
        // Clean up pools
        let cleaned = allocator.cleanup_pools();
        assert!(cleaned >= 0);
        
        // Check pool stats
        let stats = allocator.get_pool_stats("test", 1024);
    ...
    fn test_allocator_stats() {
        ...
        assert_eq!(stats.successful_allocations, 10);
