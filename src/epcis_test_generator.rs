...
impl EPCISTestDataGenerator {
    ...
    fn add_participants_to_ontology(&self, ontology: &mut Ontology) -> OwlResult<()> {
        for participant in &self.participants {
            let participant_class = Class::new(format!("http://example.org/participants/{}", participant.id));
    ...
    pub fn generate_events(&mut self) -> Vec<EPCISEvent> {
        ...
        let event_count = self.config.event_count.min(self.config.scale.event_range().1);
    ...
    fn create_event(&mut self, event_type: EPCISEventType, index: usize) -> EPCISEvent {
        ...
        let mut event = EPCISEvent {
            ...
            read_point: Some(ReadPoint {
                ...
                name: format!("Read Point at {}", participant.location.as_ref().unwrap().name),
    ...
    pub fn get_stats(&self) -> GenerationStats {
        GenerationStats {
            ...
            actual_events_generated: self.config.event_count.min(self.config.scale.event_range().1),
        }
    }
}
...
mod tests {
    ...
    fn test_small_scale_generation() {
        ...
        assert!(stats.actual_events_generated <= 500);
        assert_eq!(stats.scale, TestScale::Small);
        println!("Small scale test: {}", stats.summary());
    }
    ...
    fn test_event_type_distribution() {
        let config = TestDataConfig {
            event_count: 100,
            scale: TestScale::Small,
            include_complex_scenarios: false,
            seed: None,
        };
    ...
    fn test_scale_ranges() {
        assert_eq!(TestScale::Small.event_range(), (100, 500));
        assert_eq!(TestScale::Medium.event_range(), (1000, 5000));
        assert_eq!(TestScale::Large.event_range(), (10000, 50000));
    }
}