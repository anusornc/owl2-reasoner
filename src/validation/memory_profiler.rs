...
impl MemoryProfiler {
    ...
    pub fn profile_ontology_memory_usage(&mut self, size_factor: usize) -> OwlResult<MemoryStats> {
        ...
        let baseline_total = self
            ...
            .as_ref()
            .ok_or_else(|| {
                crate::error::OwlError::ValidationError("Baseline not taken".to_string())
            })?
            .total_allocated_mb;
        let baseline_current = self
            .baseline_stats
            .as_ref()
            .ok_or_else(|| {
                crate::error::OwlError::ValidationError("Baseline not taken".to_string())
        ...
        let baseline_allocations = self
            ...
            .allocation_count;
        let baseline_deallocations = self
            .baseline_stats
            .as_ref()
            .ok_or_else(|| {
                crate::error::OwlError::ValidationError("Baseline not taken".to_string())
        ...
        self.profiles
            .insert(format!("ontology_size_{}", size_factor), stats.clone());
        Ok(stats)
    }

    /// Profile class creation memory usage using basic entity size estimation
    fn profile_class_creation(
        &mut self,
        ...
        self.entity_profiles
            .insert(format!("classes_{}", count), profile.clone());
        Ok(profile)
    }
    ...
    fn profile_property_creation(
        ...
        self.entity_profiles
            .insert(format!("properties_{}", count), profile.clone());
        Ok(profile)
    }

    /// Profile axiom creation memory usage using basic entity size estimation
    fn profile_axiom_creation(
        &mut self,
        ontology: &mut Ontology,
        count: usize,
    ) -> OwlResult<EntityMemoryProfile> {
        ...
        self.entity_profiles
            .insert(format!("axioms_{}", count), profile.clone());
        Ok(profile)
    }
    ...
    fn profile_reasoning_operations(&self, ontology: &Ontology) -> OwlResult<MemoryStats> {
        ...
        Ok(MemoryStats {
            ...
            peak_memory_mb: after_memory
                .peak_memory_mb
                .max(before_memory.peak_memory_mb),
