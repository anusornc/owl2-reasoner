...
impl RealtimeMonitoring {
    ...
    async fn update_dashboard_data(&self) -> OwlResult<()> {
        ...
        if !all_sessions.is_empty() {
            let avg_progress = all_sessions.iter().map(|s| s.current_progress).sum::<f64>()
                / all_sessions.len() as f64;

            dashboard_data.average_progress = avg_progress;
            dashboard_data.estimated_completion_time =
                self.calculate_estimated_completion_time(&all_sessions);
    ...
    fn calculate_estimated_completion_time(&self, sessions: &[MonitoringSession]) -> Duration {
        let incomplete_sessions: Vec<_> =
            sessions.iter().filter(|s| s.end_time.is_none()).collect();
        ...
        let avg_progress_rate: f64 = incomplete_sessions
            .iter()
            .map(|s| s.current_progress)
            .sum::<f64>()
            / incomplete_sessions.len() as f64;
        ...
        let avg_time_so_far = incomplete_sessions
            .iter()
        ...
        Duration::from_secs((estimated_remaining.ceil() as u64))
    }
...
impl MetricsCollector {
    ...
    fn update_global_metrics(&mut self) {
        let completed_sessions: usize = self
            .session_metrics
            .values()
        ...
        if completed_sessions > 0 {
            ...
            self.global_metrics.average_session_duration =
                Duration::from_millis((total_duration.as_millis() / completed_sessions as u128) as u64);

            // Update most recent session
            for (session_id, latest_session) in &self.session_metrics {
                if let Some(_duration) = latest_session.total_duration {
                    // Check if this session was updated recently (within last minute)
                    // Note: In a real implementation, we'd store session end time as Instant
                    // For now, just mark the first found session as most recent
                    self.global_metrics.most_recent_session = Some(session_id.clone());
                    break;
                }
            }
...
impl EventPublisher {
    ...
    pub async fn publish_event(&mut self, event: ValidationEvent) -> OwlResult<()> {
        self.event_sender.send(event).await.map_err(|e| {
            OwlError::IOError(std::io::Error::new(
...
impl RealtimeProfiler {
    ...
    pub async fn start_profiling(&mut self, session_id: &str) -> OwlResult<String> {
        ...
        {
            let mut profiles = self.session_metrics.write().await;
    ...
    pub async fn end_profiling(&mut self, session_id: &str) -> OwlResult<PerformanceProfile> {
        ...
        if let Some(mut profile) = profiles.remove(session_id) {
            ...
            profile.performance_metrics = ProfilePerformanceMetrics {
                ...
                success_rate: profile.measurements.iter().filter(|m| m.success).count() as f64
...
impl HeartbeatMonitor {
    ...
    pub async fn start(&mut self) -> OwlResult<()> {
        ...
        *self.last_heartbeat.write().await = Instant::now();
        ...
        info!(
            "Started heartbeat monitoring for session: {}",
            self.session_id
        );
        Ok(())
    }
    ...
    pub async fn stop(&mut self) -> OwlResult<()> {
        *self.is_active.write().await = false;
        info!(
            "Stopped heartbeat monitoring for session: {}",
            self.session_id
        );
        Ok(())
    }
