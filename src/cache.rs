...
mod tests {
    ...
    fn test_cache_strategies() {
        ...
        // Test LFU strategy
        let lfu_cache = BoundedCache::<String, usize, LfuStrategy>::with_strategy(
            CacheConfigBuilder::new().max_size(3).build(),
            LfuStrategy::new().min_access_count(2),
        );

        // Test FIFO strategy
        let fifo_cache = BoundedCache::<String, usize, FifoStrategy>::with_strategy(
            CacheConfigBuilder::new().max_size(3).build(),
            FifoStrategy::new(),
        );

        // Test Random strategy
        let random_cache = BoundedCache::<String, usize, RandomStrategy>::with_strategy(
            CacheConfigBuilder::new().max_size(3).build(),
            RandomStrategy::new(),
        );
