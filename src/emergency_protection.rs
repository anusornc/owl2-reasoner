...
mod tests {
    ...
    #[test]
    fn test_emergency_protection_creation() {
        let protection = EmergencyProtection::new();
        let state = protection.get_emergency_state();
        assert_eq!(state, EmergencyState::Normal);
    }

    #[test]
    fn test_emergency_allocation_check() {
        let protection = EmergencyProtection::new();

        // Should allow normal allocations
        assert!(protection.can_emergency_allocate(1024));

        // Should allow emergency allocations
        assert!(protection.can_emergency_allocate(512));
    }

    #[test]
    fn test_emergency_config() {
        let config = EmergencyProtectionConfig {
            emergency_threshold: 0.9,
            force_cleanup_interval: Duration::from_millis(200),
            max_emergency_duration: Duration::from_secs(600),
            enable_thread_suspension: true,
            enable_emergency_cache_clearing: false,
            enable_emergency_arena_reset: false,
            emergency_allocation_limit: 2048,
        };

        let protection = EmergencyProtection::with_config(config);
        let stats = protection.get_emergency_stats();
        assert_eq!(stats.current_state, EmergencyState::Normal);
    }
    ...
    fn test_emergency_stats() {
        ...
        assert_eq!(stats.current_state, EmergencyState::Normal);
        assert_eq!(stats.emergency_duration, Duration::ZERO);
        assert_eq!(stats.total_events, 0);
    }

    #[test]
    fn test_emergency_actions() {
        let mut protection = EmergencyProtection::new();

        // Test triggering an emergency action
        let result = protection.trigger_emergency_action(
            EmergencyAction::EmergencyGC,
            "Test emergency action".to_string(),
        );
        ...
        assert!(stats.action_counts.contains_key(&EmergencyAction::EmergencyGC));
    }
    ...
    fn test_emergency_history() {
        ...
        let _ = protection.trigger_emergency_action(
            EmergencyAction::ForceCacheCleanup,
            "Test 1".to_string(),
        );
        let _ = protection.trigger_emergency_action(
        ...
        let history = protection.get_emergency_history();
