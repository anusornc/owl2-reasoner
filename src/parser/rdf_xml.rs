...
use crate::iri::IRI;
use crate::entities::*;
use crate::axioms::{SubClassOfAxiom, EquivalentClassesAxiom, DisjointClassesAxiom,
                   ClassAssertionAxiom, PropertyAssertionAxiom, SubObjectPropertyAxiom,
                   EquivalentObjectPropertiesAxiom, DisjointObjectPropertiesAxiom, Axiom};
...
impl RdfXmlParser {
    ...
    fn parse_content(&mut self, content: &str) -> OwlResult<Ontology> {
        ...
        // Parse XML document with proper structure
        let document = self.parse_xml_document(content)?;

        // Process RDF/XML structure
        self.process_rdf_document(&mut ontology, &document)?;

        // Process collected resource information
    ...
    fn parse_xml_declaration(&self, chars: &mut std::str::CharIndices<'_>, document: &mut XmlDocument) -> OwlResult<()> {
    ...
    fn parse_doctype(&self, chars: &mut std::str::CharIndices<'_>, document: &mut XmlDocument) -> OwlResult<()> {
        ...
        while let Some((_, c)) = chars_clone.next() {
            if c == '<' {
                ...
                while let Some((_, c)) = chars_clone.next() {
                    ...
                    if doctype_check.len() >= 8 && doctype_check.iter().collect::<String>().to_lowercase().ends_with("doctype") {
                        found_doctype = true;
                        break;
                    }
                    if doctype_check.len() > 20 { // Safety valve
                        break;
                    }
                }
                break;
        ...
        while let Some((_, c)) = chars.next() {
            ...
            if doctype_start.len() >= 8 && doctype_start.iter().collect::<String>().to_lowercase().ends_with("doctype") {
                break;
            }
            if doctype_start.len() > 20 { // Safety valve
                break;
            }
        }

        if doctype_start.iter().collect::<String>().to_lowercase().ends_with("doctype") {
            ...
            while let Some((_, c)) = chars.next() {
                if c == '[' {
                    ...
                    bracket_count -= 1;
                    if bracket_count == 0 && chars.clone().next().map_or(false, |(_, next)| next == '>') {
                        chars.next(); // Consume '>'
                        break;
                    }
    ...
    fn parse_element(&mut self, chars: &mut std::str::CharIndices<'_>) -> OwlResult<Option<XmlElement>> {
    ...
    fn parse_xml_attributes(&mut self, chars: &mut std::str::CharIndices<'_>,
                           attributes: &mut HashMap<String, String>, is_empty: &mut bool) -> OwlResult<()> {
    ...
    fn parse_element_content(&mut self, chars: &mut std::str::CharIndices<'_>, element: &mut XmlElement) -> OwlResult<()> {
        ...
        if let Some(root) = &document.root {
            ...
            for (key, value) in &root.attributes {
                if key == "xml:base" {
                    ...
                    self.namespaces.insert(prefix.to_string(), value.to_string());
                } else if key == "xmlns" {
    ...
    fn process_rdf_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        ...
        match element_name {
            ...
            _ => {
                self.process_unknown_element(ontology, element)
            },
        }
    }
    ...
    fn process_class_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let Some(about) = element.attributes.get("rdf:about") {
            ...
            let class_iri_str = class_iri.to_string();
    ...
    fn process_data_property_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let Some(about) = element.attributes.get("rdf:about") {
            let prop_iri = self.resolve_iri(about)?;
    ...
    fn process_restriction_element(&mut self, _ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        let mut on_property = None;
        let mut restriction_type = None;
        let mut restriction_value = None;
        ...
        for child in &element.children {
            match child.name.as_str() {
                ...
                "owl:allValuesFrom" | "owl:someValuesFrom" => {
                    if let Some(resource) = child.attributes.get("rdf:resource") {
                        restriction_type = Some(child.name.clone());
                        restriction_value = Some(ResourceValue::Resource(self.resolve_iri(resource)?));
                    }
                }
                "owl:hasValue" => {
                    if let Some(resource) = child.attributes.get("rdf:resource") {
                ...
                "owl:minCardinality" | "owl:maxCardinality" | "owl:cardinality" => {
                    if let Some(content) = child.attributes.get("rdf:datatype") {
                        restriction_type = Some(child.name.clone());
                        // Parse cardinality from content or attributes
                        if let Some(card_str) = child.attributes.get("rdf:datatype") {
        ...
        if let Some(prop_iri) = on_property {
                ...
                if child.name.trim_start_matches('<') == "rdf:type" {
                    if let Some(resource) = child.attributes.get("rdf:resource") {
                        ...
                        if type_iri.as_str() == "http://www.w3.org/2002/07/owl#Class" {
                            ...
                            ontology.add_class(class)?;
                        } else if type_iri.as_str() == "http://www.w3.org/2002/07/owl#ObjectProperty" {
                            let prop = ObjectProperty::new(subject_iri.clone());
                            ontology.add_object_property(prop)?;
                        } else if type_iri.as_str() == "http://www.w3.org/2002/07/owl#DatatypeProperty" {
                            let prop = DataProperty::new(subject_iri.clone());
                            ontology.add_data_property(prop)?;
                        } else if type_iri.as_str() == "http://www.w3.org/2002/07/owl#NamedIndividual" {
    ...
    fn process_subclass_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_equivalent_class_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_disjoint_class_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_subproperty_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_equivalent_property_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_disjoint_property_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(subject), Some(object)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_type_element(&mut self, ontology: &mut Ontology, element: &XmlElement) -> OwlResult<()> {
        if let (Some(individual), Some(class_iri)) = (self.get_element_subject(element), self.get_element_object(element)) {
    ...
    fn process_intersection_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:intersectionOf
        Ok(())
    }

    fn process_union_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:unionOf
        Ok(())
    }

    fn process_complement_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:complementOf
        Ok(())
    }

    fn process_oneof_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:oneOf
        Ok(())
    }

    fn process_all_values_from_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:allValuesFrom
        Ok(())
    }

    fn process_some_values_from_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:someValuesFrom
        Ok(())
    }

    fn process_has_value_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:hasValue
        Ok(())
    }

    fn process_min_cardinality_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:minCardinality
        Ok(())
    }

    fn process_max_cardinality_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:maxCardinality
        Ok(())
    }

    fn process_exact_cardinality_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Implementation for owl:cardinality
        Ok(())
    }

    fn process_unknown_element(&mut self, _ontology: &mut Ontology, _element: &XmlElement) -> OwlResult<()> {
        // Silently ignore unknown elements or handle them as needed
        Ok(())
    }

    /// Process class properties
    fn process_class_property(&mut self, element: &XmlElement, resource_info: &mut ResourceInfo) -> OwlResult<()> {
        let element_name = element.name.trim_start_matches('<');
        match element_name {
            "rdfs:subClassOf" => {
                if let Some(resource) = element.attributes.get("rdf:resource") {
                    ...
                    let prop = ResourceProperty {
                        property_iri: IRI::new("http://www.w3.org/2000/01/rdf-schema#subClassOf").unwrap(),
                        value: ResourceValue::Resource(super_class_iri),
                    };
                    resource_info.properties.entry("subClassOf".to_string())
            ...
            "owl:equivalentClass" => {
                if let Some(resource) = element.attributes.get("rdf:resource") {
                    ...
                    let prop = ResourceProperty {
                        property_iri: IRI::new("http://www.w3.org/2002/07/owl#equivalentClass").unwrap(),
                        value: ResourceValue::Resource(equiv_class_iri),
                    };
                    resource_info.properties.entry("equivalentClass".to_string())
                        .or_insert_with(Vec::new)
                        .push(prop);
                }
            }
    ...
    /// Process individual properties
    fn process_individual_property(&mut self, ontology: &mut Ontology, individual_iri: &IRI, element: &XmlElement) -> OwlResult<()> {
        let property_iri = self.resolve_qname(&element.name)?;

        if let Some(resource) = element.attributes.get("rdf:resource") {
            // Object property
            let object_iri = self.resolve_iri(resource)?;
            let axiom = PropertyAssertionAxiom::new(individual_iri.clone(), property_iri, object_iri);
    ...
    /// Process property characteristics
    fn process_property_characteristic(&mut self, _ontology: &mut Ontology, _prop_iri: &IRI, _char_iri: &IRI) -> OwlResult<()> {
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    <owl:NamedIndividual rdf:about="http://example.org/john"/>
        ...
        if let Ok(ontology) = result {
            ...
            assert_eq!(ontology.named_individuals().len(), 1, "Should have parsed 1 individual");
    ...
    <owl:Class rdf:about="http://example.org/Animal"/>
        ...
        if let Ok(ontology) = result {
            ...
            let subclass_count = ontology.axioms().iter().filter(|axiom| {
                matches!(***axiom, crate::axioms::Axiom::SubClassOf(_))
            }).count();
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    <owl:Class rdf:about="http://example.org/Human"/>
        ...
        if let Ok(ontology) = result {
            ...
            let equiv_count = ontology.axioms().iter().filter(|axiom| {
                matches!(***axiom, crate::axioms::Axiom::EquivalentClasses(_))
            }).count();
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    <rdf:Description rdf:about="http://example.org/john">
        ...
        if let Ok(ontology) = result {
            ...
            let assertion_count = ontology.axioms().iter().filter(|axiom| {
                matches!(***axiom, crate::axioms::Axiom::ClassAssertion(_))
            }).count();
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    <rdf:Description rdf:about="http://example.org/john">
        ...
        if let Ok(ontology) = result {
            ...
            let prop_assertion_count = ontology.axioms().iter().filter(|axiom| {
                matches!(***axiom, crate::axioms::Axiom::PropertyAssertion(_))
            }).count();

            assert!(prop_assertion_count > 0, "Should have property assertion axioms");
        }
    }
    ...
    fn test_rdf_xml_with_config() {
        ...
        config.prefixes.insert("test".to_string(), "http://test.org/".to_string());
