...
mod tests {
    ...
    fn test_grammar_creation() {
        let grammar = ManchesterGrammar::new();
        assert!(grammar.rules.len() > 0);
        assert_eq!(grammar.start_symbol, Production::Document);
    }
    ...
    fn test_precedence_levels() {
        let grammar = ManchesterGrammar::new();
        let not_precedence = grammar.get_precedence("not");
        let and_precedence = grammar.get_precedence("and");
        let or_precedence = grammar.get_precedence("or");

        assert!(not_precedence.unwrap() > and_precedence.unwrap());
        assert!(and_precedence.unwrap() > or_precedence.unwrap());
    }
    ...
    fn test_parser_state() {
        ...
        state.push(Symbol::Terminal(TokenType::Class), 1);
    ...
    #[test]
    fn test_grammar_statistics() {
        let grammar = ManchesterGrammar::new();
        let stats = grammar.statistics();

        assert!(stats.total_rules > 0);
        assert!(stats.total_productions > 0);
        assert!(stats.max_rule_length > 0);
        assert!(stats.average_rule_length > 0.0);
    }
}
