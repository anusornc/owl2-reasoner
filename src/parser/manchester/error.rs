...
mod tests {
    ...
    fn test_parse_error_creation() {
        let error =
            ParseError::unexpected_token(vec!["Class", "ObjectProperty"], "Individual", 1, 10);
    ...
    fn test_error_context() {
        let mut context = ErrorContext::new("Class: Person".to_string());
        context.add_prefix("ex", "http://example.org/");

        assert_eq!(
            context.resolve_prefix("ex:Person"),
            Some("http://example.org/Person".to_string())
        );
        assert_eq!(context.get_line_text(1), Some("Class: Person"));
    }

    #[test]
    fn test_error_reporter() {
        let context = ErrorContext::new("Class: Person".to_string());
        let mut reporter = ErrorReporter::new(context);

        reporter.report_error(ParseError::syntax_error("Test error", 1, 5));
        reporter.report_warning("Test warning");

        assert_eq!(reporter.errors().error_count(), 1);
        assert_eq!(reporter.errors().warning_count(), 1);
    }
}
