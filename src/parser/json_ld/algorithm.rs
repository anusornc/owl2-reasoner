...
mod tests {
    ...
    #[test]
    fn test_expand_simple_context() {
        let mut algorithm = JsonLdExpansionAlgorithm::new();
        ...
        let result = algorithm.expand(&input);
        assert!(result.is_ok());
    }

    #[test]
    fn test_expand_with_graph() {
        let mut algorithm = JsonLdExpansionAlgorithm::new();
