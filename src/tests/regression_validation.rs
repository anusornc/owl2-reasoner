...
memory_safe_test!(
    ...
    MemorySafeTestConfig::small(),
    {
        ...
        let is_employee_person = reasoner
            .is_subclass_of(employee.iri(), person.iri())
            .unwrap();
        let is_manager_employee = reasoner
            .is_subclass_of(manager.iri(), employee.iri())
            .unwrap();
        let is_manager_person = reasoner
            .is_subclass_of(manager.iri(), person.iri())
            .unwrap();
        ...
        assert!(
            is_manager_person,
            "Manager should be subclass of Person (transitive)"
        );
...
memory_safe_test!(
    ...
    MemorySafeTestConfig::small(),
    {
        ...
        assert!(
            after_stats.iri_hits + after_stats.iri_misses
...
memory_safe_test!(
    ...
    MemorySafeTestConfig::small(),
    {
        ...
        let mut symmetric_prop =
            ObjectProperty::new(Arc::new(IRI::new("http://example.org/symmetric").unwrap()));
        symmetric_prop.add_characteristic(ObjectPropertyCharacteristic::Symmetric);

        let mut transitive_prop =
            ObjectProperty::new(Arc::new(IRI::new("http://example.org/transitive").unwrap()));
        transitive_prop.add_characteristic(ObjectPropertyCharacteristic::Transitive);

        let mut asymmetric_prop =
            ObjectProperty::new(Arc::new(IRI::new("http://example.org/asymmetric").unwrap()));
        asymmetric_prop.add_characteristic(ObjectPropertyCharacteristic::Asymmetric);

        // Add properties to ontology
        ontology
            .add_object_property(symmetric_prop.clone())
            .unwrap();
        ontology
            .add_object_property(transitive_prop.clone())
            .unwrap();
        ontology
            .add_object_property(asymmetric_prop.clone())
            .unwrap();
...
memory_safe_test!(
    ...
    MemorySafeTestConfig::small(),
    {
        ...
        assert!(
            is_consistent,
            "Ontology with individuals should be consistent"
        );
        ...
        assert!(
            !employee_instances.is_empty(),
            "Should have employee instances"
        );
...
memory_safe_test!(
    ...
    MemorySafeTestConfig::medium(),
    {
        ...
        assert!(
            is_consistent,
            "Ontology with complex expressions should be consistent"
        );
...
memory_safe_test!(
    ...
    MemorySafeTestConfig::medium(),
    {
        ...
        assert!(
            !instances.is_empty(),
            "Instance retrieval should still work"
        );
...
memory_safe_test!(
    test_regression_validation_summary,
