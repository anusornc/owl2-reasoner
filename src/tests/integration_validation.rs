...
memory_safe_test!(test_memory_ontology_integration, MemorySafeTestConfig::medium(), {
    ...
    let is_manager_employee = reasoner.is_subclass_of(manager_class.iri(), employee_class.iri()).unwrap();
    let is_employee_person = reasoner.is_subclass_of(employee_class.iri(), person_class.iri()).unwrap();
    ...
    println!("    Memory usage: {:.1} MB", report.end_memory as f64 / 1024.0 / 1024.0);
...
memory_safe_test!(test_cache_memory_integration, MemorySafeTestConfig::small(), {
    ...
    println!("  Initial cache stats: {} hits, {} misses", 
...
memory_safe_test!(test_parser_memory_integration, MemorySafeTestConfig::medium(), {
...
"#;
    ...
    println!("    Classes parsed: {}", reasoner.ontology().classes().len());
    println!("    Properties parsed: {}", reasoner.ontology().object_properties().len());
    println!("    Memory usage: {:.1} MB", report.end_memory as f64 / 1024.0 / 1024.0);
...
memory_safe_test!(test_reasoning_memory_integration, MemorySafeTestConfig::medium(), {
    ...
    for (subclass, superclass) in &relationships {
        let subclass_class = class_entities.iter()
            .find(|(name, _)| *name == *subclass)
            .unwrap().1.clone();
        let superclass_class = class_entities.iter()
    ...
    for (subclass, superclass) in &relationships {
        let subclass_iri = class_entities.iter()
            .find(|(name, _)| *name == *subclass)
            .unwrap().1.iri();
        let superclass_iri = class_entities.iter()
            .find(|(name, _)| *name == *superclass)
            .unwrap().1.iri();
        
        let is_subclass = reasoner.is_subclass_of(subclass_iri, superclass_iri).unwrap();
        assert!(is_subclass, "{} should be subclass of {}", subclass, superclass);
    }
    
    // Test instance retrieval
    let person_class = class_entities.iter()
...
memory_safe_test!(test_error_handling_memory_integration, MemorySafeTestConfig::small(), {
    ...
    assert!(report.is_acceptable(), "Memory usage should be acceptable despite errors");
});

/// Test concurrent integration of multiple components
memory_safe_test!(test_concurrent_component_integration, MemorySafeTestConfig::large(), {
    ...
    for thread_id in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            ...
            if thread_id == 0 {
                ...
                let mut ontology = Ontology::new();
                for i in 0..100 {
                    let iri = IRI::new(&format!("http://example.org/thread0/class{}", i)).unwrap();
                    let class = Class::new(Arc::new(iri));
                ...
                let mut iris = Vec::new();
                for i in 0..200 {
                    let iri = IRI::new(&format!("http://example.org/thread1/cache{}", i)).unwrap();
                    iris.push(iri);
                }
...
"#;
    ...
    for (thread_id, report) in results.iter() {
        println!("    Thread {}: {:.1}% memory usage, {} warnings", 
                 thread_id,
                 (report.end_memory as f64 / report.max_memory_bytes as f64) * 100.0,
                 report.warnings.len());
        
        assert!(report.is_acceptable(), "Each thread should have acceptable memory usage");
    }
});

/// Test full pipeline integration
memory_safe_test!(test_full_pipeline_integration, MemorySafeTestConfig::large(), {
...
"#;
    ...
    let managers_are_employees = reasoner.is_subclass_of(&manager_iri, &employee_iri).unwrap();
    ...
    println!("    Parsed successfully: {}", !ontology.classes().is_empty());
    println!("    Classes: {}", reasoner.ontology().classes().len());
    println!("    Properties: {}", reasoner.ontology().object_properties().len());
    println!("    Individuals: {}", reasoner.ontology().named_individuals().len());
    ...
    assert!(report.is_acceptable(), "Memory usage should be acceptable for full pipeline");
});

/// Comprehensive integration validation summary
memory_safe_test!(test_integration_validation_summary, MemorySafeTestConfig::large(), {
    ...
    assert!(final_leak_report.memory_efficiency_score > 0.6, "System should have good efficiency");
