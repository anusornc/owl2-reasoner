...
memory_safe_test!(test_library_documentation_examples, MemorySafeTestConfig::medium(), {
    ...
    assert!(is_subclass, "Parent should be subclass of Person as documented");
...
memory_safe_test!(test_readme_examples, MemorySafeTestConfig::medium(), {
    ...
    let person = Class::new(Arc::new(IRI::new("http://example.org/family/Person").unwrap()));
    let male = Class::new(Arc::new(IRI::new("http://example.org/family/Male").unwrap()));
    let female = Class::new(Arc::new(IRI::new("http://example.org/family/Female").unwrap()));
    let parent = Class::new(Arc::new(IRI::new("http://example.org/family/Parent").unwrap()));
    ...
    // Create individuals
    let john = NamedIndividual::new(Arc::new(IRI::new("http://example.org/family/John").unwrap()));
    let mary = NamedIndividual::new(Arc::new(IRI::new("http://example.org/family/Mary").unwrap()));
    ...
    ontology.add_class_assertion(
        ClassAssertionAxiom::new(john.iri().clone(), ClassExpression::Class(male.clone()))
    ).unwrap();
    
    ontology.add_class_assertion(
        ClassAssertionAxiom::new(mary.iri().clone(), ClassExpression::Class(female.clone()))
    ).unwrap();
    ...
    assert_eq!(males.len() + females.len(), people.len(), "All males and females should be people");
    
    println!("  ✅ README examples work correctly");
});

/// Test that example files compile and run
memory_safe_test!(test_example_files_compilation, MemorySafeTestConfig::large(), {
        ...
        for entry in entries.flatten() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("rs") {
                ...
                if let Ok(content) = fs::read_to_string(&path) {
                    ...
                    assert!(content.contains("fn") || content.contains("use"), 
...
memory_safe_test!(test_turtle_parsing_documentation, MemorySafeTestConfig::medium(), {
...
"#;
    ...
    assert!(!ontology.subclass_axioms().is_empty(), "Should have parsed subclass axioms");
...
memory_safe_test!(test_error_handling_documentation, MemorySafeTestConfig::small(), {
...
memory_safe_test!(test_memory_safety_documentation, MemorySafeTestConfig::medium(), {
    ...
    assert!(initial_stats.total_usage > 0, "Should have non-zero memory usage");
    ...
    assert!(check_result.is_ok(), "Memory check should succeed with normal usage");
    ...
    assert!(leak_report.memory_efficiency_score >= 0.0 && leak_report.memory_efficiency_score <= 1.0);
    
    println!("  Memory efficiency score: {:.2}", leak_report.memory_efficiency_score);
...
memory_safe_test!(test_performance_documentation, MemorySafeTestConfig::medium(), {
    ...
    for i in 1..500 {
        ...
        let superclass_iri = IRI::new(&format!("http://example.org/perf/class{}", i / 2)).unwrap();
    ...
    assert!(creation_time.as_secs() < 10, "Ontology creation should be fast");
    assert!(reasoning_time.as_secs() < 15, "Reasoning should be fast");
    assert!(is_consistent, "Performance test ontology should be consistent");
    
    println!("  Performance metrics:");
    println!("    Creation: {:?}, Reasoning: {:?}", creation_time, reasoning_time);
    println!("  ✅ Performance documentation examples work correctly");
});

/// Test that API reference documentation works
memory_safe_test!(test_api_reference_documentation, MemorySafeTestConfig::small(), {
    ...
    // Test Ontology API
    let mut ontology = Ontology::new();
    assert!(ontology.classes().is_empty(), "New ontology should have no classes");
    assert!(ontology.object_properties().is_empty(), "New ontology should have no properties");
    ...
    assert!(!ontology.classes().is_empty(), "Ontology should now have classes");
    assert!(ontology.classes().contains(&class), "Should contain added class");
    ...
    assert!(cache_stats.iri_hits >= 0, "Cache hits should be non-negative");
    assert!(cache_stats.iri_misses >= 0, "Cache misses should be non-negative");
    
    println!("  ✅ API reference documentation examples work correctly");
});

/// Test advanced features documentation
memory_safe_test!(test_advanced_features_documentation, MemorySafeTestConfig::medium(), {
    ...
    // Create base classes
    let person = Class::new(Arc::new(IRI::new("http://example.org/advanced/Person").unwrap()));
    let student = Class::new(Arc::new(IRI::new("http://example.org/advanced/Student").unwrap()));
    let teacher = Class::new(Arc::new(IRI::new("http://example.org/advanced/Teacher").unwrap()));
    ...
    assert!(is_consistent, "Advanced features ontology should be consistent");
    
    println!("  ✅ Advanced features documentation examples work correctly");
});

/// Test that all documentation is accessible and functional
memory_safe_test!(test_documentation_accessibility, MemorySafeTestConfig::small(), {
    ...
    for doc_file in &doc_files {
        if Path::new(doc_file).exists() {
            ...
            println!("  ⚠️  {} does not exist (may be expected in test environment)", doc_file);
        }
    }
    
    // At least some documentation should be accessible
    assert!(accessible_docs > 0, "At least some documentation should be accessible");
...
memory_safe_test!(test_documentation_verification_summary, MemorySafeTestConfig::large(), {
    ...
    assert!(final_stats.pressure_level < 0.8, "System should be ready for documentation");
