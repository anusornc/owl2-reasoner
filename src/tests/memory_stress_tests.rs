...
memory_safe_stress_test!(test_extreme_memory_pressure, {
    ...
    println!("    Peak memory: {:.1} MB", report.peak_memory as f64 / 1024.0 / 1024.0);
    println!("    Warnings: {}", report.warnings.len());
    println!("    Cleanups: {}", report.cleanup_count);
    
    // Should have warnings due to high memory usage
    assert!(!report.warnings.is_empty(), "Should have memory warnings under stress");
...
memory_safe_stress_test!(test_concurrent_memory_stress, {
    ...
    let total_guard_warnings: usize = results.iter()
    ...
    for (thread_id, report, _) in results.iter() {
        println!("    Thread {}: {:.1}% peak usage, {} warnings", 
                 thread_id,
    ...
    for (_, report, _) in results.iter() {
        assert!(report.end_memory > 0, "Each thread should have memory usage");
    }
});
...
memory_safe_stress_test!(test_memory_limit_enforcement, {
    ...
    println!("    Final memory usage: {:.1} MB", 
             report.end_memory as f64 / 1024.0 / 1024.0);
    println!("    Warnings: {}", report.warnings.len());
    
    // Should have hit the limit
    assert!(limit_exceeded_count > 0, "Should have exceeded memory limit");
    
    // Should have warnings
    assert!(!report.warnings.is_empty(), "Should have warnings about limit");
...
memory_safe_stress_test!(test_memory_leak_detection_stress, {
    ...
    println!("  Initial efficiency score: {:.2}", initial_report.memory_efficiency_score);
    ...
    for i in 0..100 {
        ...
        if i % 20 == 0 {
            ...
            println!("  Allocation {} - Efficiency: {:.2}", i, mid_report.memory_efficiency_score);
        }
    }
    
    let stressed_report = detect_memory_leaks();
    println!("  Under stress - Efficiency: {:.2}", stressed_report.memory_efficiency_score);
    println!("  Potential leaks detected: {}", stressed_report.potential_leaks.len());
    
    // Clean up half the allocations
    large_allocations.drain(0..50);
    
    let partial_cleanup_report = detect_memory_leaks();
    println!("  Partial cleanup - Efficiency: {:.2}", partial_cleanup_report.memory_efficiency_score);
    ...
    println!("  Final cleanup - Efficiency: {:.2}", final_report.memory_efficiency_score);
    
    // Efficiency should improve after cleanup
    assert!(final_report.memory_efficiency_score >= partial_cleanup_report.memory_efficiency_score,
...
memory_safe_stress_test!(test_cache_memory_stress, {
    ...
    for i in 10000..20000 {
        ...
        if i % 1000 == 0 {
            ...
            let cache_stats = global_cache_stats();
            println!("  Under pressure {} - Cache size: {}", i, 
                     cache_stats.iri_hits + cache_stats.iri_misses);
        }
    }
    ...
    println!("  After cache clear - Cache size: {}", 
    ...
    for ont_idx in 0..5 {
        ...
        for class_idx in 0..2000 {
            let iri = IRI::new(&format!(
                "http://example.org/ont{}/class{}", ont_idx, class_idx
            )).unwrap();
        ...
        for prop_idx in 0..500 {
            let iri = IRI::new(&format!(
                "http://example.org/ont{}/prop{}", ont_idx, prop_idx
            )).unwrap();
        ...
        for rel_idx in 0..3000 {
            let subclass_iri = IRI::new(&format!(
                "http://example.org/ont{}/class{}", ont_idx, rel_idx % 2000
            )).unwrap();
            let superclass_iri = IRI::new(&format!(
                "http://example.org/ont{}/class{}", ont_idx, (rel_idx + 1) % 2000
            )).unwrap();
            
            let subclass = crate::ClassExpression::Class(
                crate::Class::new(Arc::new(subclass_iri))
            );
            let superclass = crate::ClassExpression::Class(
                crate::Class::new(Arc::new(superclass_iri))
            );
        ...
        let _ = guard.check_memory();
        let usage = guard.memory_usage_percent();
        println!("    Ontology {} created: {:.1}% memory usage", ont_idx, usage);
...
memory_safe_stress_test!(test_rapid_allocation_cycles, {
    ...
    assert!(report.warnings.len() < 20, "Should not have excessive warnings");
});
...
memory_safe_stress_test!(test_memory_stress_summary, {
    ...
    assert!(final_stats.pressure_level < 0.95, "System pressure should be manageable after stress");
    assert!(final_leak_report.memory_efficiency_score > 0.3, "System should maintain reasonable efficiency");
});