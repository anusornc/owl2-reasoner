...
memory_safe_test!(test_basic_memory_monitoring, MemorySafeTestConfig::small(), {
    ...
    println!("  Initial: {} bytes, {:.2}% pressure", 
             initial_stats.total_usage, initial_stats.pressure_level * 100.0);
    println!("  After: {} bytes, {:.2}% pressure", 
             after_stats.total_usage, after_stats.pressure_level * 100.0);
});

/// Test memory guard configuration and behavior
memory_safe_test!(test_memory_guard_configuration, MemorySafeTestConfig::small(), {
...
memory_safe_test!(test_memory_guard_low_memory, {
    ...
    println!("Low memory test passed with {} warnings", report.warnings.len());
...
memory_safe_test!(test_memory_cleanup_functionality, MemorySafeTestConfig::medium(), {
    // Fill caches first
    let cache_stats_before = global_cache_stats();
    println!("Cache stats before cleanup: {} hits, {} misses", 
    ...
    assert!(cleanup_count > 0, "Should have performed cleanup operations");
    ...
    assert!(leak_report.memory_efficiency_score >= 0.0 && 
            leak_report.memory_efficiency_score <= 1.0);
    
    println!("Leak detection report:");
    println!("  Efficiency score: {:.2}", leak_report.memory_efficiency_score);
...
memory_safe_test!(test_concurrent_memory_access, MemorySafeTestConfig::medium(), {
    let num_threads = 4;
    ...
    for (thread_id, report) in results.iter() {
        println!("  Thread {}: {:.1}% usage, {} warnings", 
                 thread_id, 
    ...
    for (_, report) in results.iter() {
        assert!(report.is_acceptable(), "All thread reports should be acceptable");
    }
});

/// Test memory pressure detection and response
memory_safe_test!(test_memory_pressure_detection, MemorySafeTestConfig::large(), {
    ...
    println!("Memory pressure after allocation: {:.2}%", after_allocation_pressure * 100.0);
    ...
    assert!(cleanup_result.is_ok(), "Pressure-based cleanup should succeed");
...
memory_safe_test!(test_memory_guard_error_handling, MemorySafeTestConfig::small(), {
    let config = TestMemoryConfig {
    ...
    let result = guard.check_memory();
    assert!(result.is_err(), "Memory check should fail when limit exceeded");
    ...
    assert!(!report.warnings.is_empty(), "Should have warnings about limit exceeded");
    assert!(!report.is_acceptable(), "Report should not be acceptable due to limit exceeded");
    
    println!("Error handling test passed with {} warnings", report.warnings.len());
...
memory_safe_test!(test_memory_leak_detection_accuracy, MemorySafeTestConfig::medium(), {
    // Get baseline leak detection
    let baseline_report = detect_memory_leaks();
    println!("Baseline leak detection:");
    println!("  Efficiency score: {:.2}", baseline_report.memory_efficiency_score);
    println!("  Potential leaks: {}", baseline_report.potential_leaks.len());
    ...
    println!("  Efficiency score: {:.2}", after_leaks_report.memory_efficiency_score);
    println!("  Potential leaks: {}", after_leaks_report.potential_leaks.len());
    
    // The efficiency score should have decreased
    assert!(after_leaks_report.memory_efficiency_score <= baseline_report.memory_efficiency_score,
    ...
    println!("  Efficiency score: {:.2}", cleanup_report.memory_efficiency_score);
    println!("  Potential leaks: {}", cleanup_report.potential_leaks.len());
    
    // Efficiency should improve after cleanup
    assert!(cleanup_report.memory_efficiency_score >= after_leaks_report.memory_efficiency_score,
            "Efficiency score should improve after cleanup");
});

/// Test memory monitoring configuration updates
memory_safe_test!(test_memory_monitor_configuration_updates, MemorySafeTestConfig::small(), {
    ...
    let cleanup_result = monitor.check_and_cleanup();
    assert!(cleanup_result.is_ok(), "Cleanup should succeed with updated config");
    
    println!("Configuration update test passed");
});

/// Comprehensive memory safety validation summary
memory_safe_test!(test_memory_safety_validation_summary, MemorySafeTestConfig::large(), {
    ...
    println!("   Efficiency score: {:.2}", final_leak_report.memory_efficiency_score);
    println!("   Total cleanups: {}", final_stats.cleanup_count);
    
    // Assert system is in good state
    assert!(final_stats.pressure_level < 0.9, "System pressure should be manageable");
    assert!(final_leak_report.memory_efficiency_score > 0.5, "System efficiency should be reasonable");
