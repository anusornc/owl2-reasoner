...
mod tests {
    ...
    fn test_simple_consistent_ontology() {
        ...
        assert!(
            result.is_ok(),
            "Simple consistent ontology should not error"
        );
    ...
    fn test_malformed_turtle_triples() {
        ...
        for triple in malformed_triples {
            ...
            match result {
                ...
                Err(err) => {
                    panic!(
                        "Unexpected error type for malformed triple '{}': {:?}",
                        triple, err
                    );
                }
            }
    ...
    fn test_duplicate_class_addition() {
        ...
        assert!(
            result2.is_ok(),
            "Duplicate class addition should be handled gracefully"
        );
    }
    ...
    fn test_reasoning_cache_operations() {
        ...
        assert!(
            result.is_ok(),
            "Cache operations should not affect reasoning"
        );
    ...
    fn test_memory_intensive_operations() {
        ...
        assert!(
            result.is_ok(),
            "Reasoning with memory-intensive ontology should not error"
        );
        assert!(
            result.unwrap(),
            "Memory-intensive ontology should be consistent"
        );
    }
    ...
    fn test_ontology_validation_errors() {
        ...
        match result {
            ...
            Err(err) => {
                panic!(
                    "Unexpected error type for axiom with non-existent entities: {:?}",
                    err
                );
            }
        }
    ...
    fn test_graceful_error_handling() {
        ...
        match (class_result, individual_result, consistency_result) {
            ...
            (class_res, individual_res, consistency_res) => {
                ...
                println!(
                    "Some operations failed: class={:?}, individual={:?}, consistency={:?}",
                    class_res, individual_res, consistency_res
                );
            }
        }
