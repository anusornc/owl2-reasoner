...
fn bench_memory_stats_collection(c: &mut Criterion) {
    ...
    group.bench_function("is_under_memory_pressure", |b| {
        b.iter(|| {
            black_box(is_under_memory_pressure())
        })
    });
    
    group.bench_function("detect_memory_leaks", |b| {
        b.iter(|| {
            black_box(detect_memory_leaks())
        })
    });
...
fn bench_memory_guard_overhead(c: &mut Criterion) {
    ...
    group.bench_function("guard_memory_usage_percent", |b| {
        ...
        b.iter(|| {
            black_box(guard.memory_usage_percent())
        })
    });
    ...
    group.bench_function("realistic_scenario_with_safety", |b| {
        b.iter(|| {
            ...
            for i in 1..200 {
                ...
                let subclass = owl2_reasoner::ClassExpression::Class(
                    owl2_reasoner::Class::new(Arc::new(subclass_iri))
                );
                let superclass = owl2_reasoner::ClassExpression::Class(
                    owl2_reasoner::Class::new(Arc::new(superclass_iri))
                );
