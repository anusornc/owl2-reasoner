...
pub fn bench_large_ontology_handling(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000, 20000].iter() {
        group.bench_with_input(BenchmarkId::new("large_ontology_reasoning", size), size, |b, size| {
            b.iter(|| {
...
pub fn bench_ontology_loading(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000, 20000].iter() {
        group.bench_with_input(BenchmarkId::new("load_large_ontology", size), size, |b, size| {
            b.iter(|| {
                let ontology = create_large_ontology(*size);
                black_box(ontology);
            })
...
pub fn bench_deep_hierarchy_reasoning(c: &mut Criterion) {
    ...
    for depth in [10, 50, 100, 200].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("deep_consistency", depth), depth, |b, _| {
            b.iter(|| {
                let result = reasoner.is_consistent();
                black_box(result);
            })
...
pub fn bench_wide_hierarchy_reasoning(c: &mut Criterion) {
    ...
    for width in [10, 50, 100, 200].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("wide_consistency", width), width, |b, _| {
            b.iter(|| {
                let result = reasoner.is_consistent();
                black_box(result);
            })
...
pub fn bench_concurrent_access(c: &mut Criterion) {
    ...
    for thread_count in [2, 4, 8].iter() {
        group.bench_with_input(BenchmarkId::new("concurrent_reasoning", thread_count), thread_count, |b, thread_count| {
            b.iter(|| {
