use owl2_reasoner::axioms::{
    PropertyAssertionAxiom, SameIndividualAxiom, DifferentIndividualsAxiom, Axiom,
};
...
fn test_same_individual() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let axiom1 = PropertyAssertionAxiom::new(
        clark_kent.clone(),
        has_power.clone(),
        flight.clone(),
    );
...
fn test_different_individuals() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let different_axiom = DifferentIndividualsAxiom::new(vec![
        batman.clone(),
        bruce_wayne.clone(),
        joker.clone(),
    ]);
...
fn test_conflicting_equality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let same_axiom1 = SameIndividualAxiom::new(vec![
        peter_parker.clone(),
        spiderman.clone(),
    ]);
    ...
    let same_axiom2 = SameIndividualAxiom::new(vec![
        miles_morales.clone(),
        spiderman.clone(),
    ]);
