use owl2_reasoner::axioms::{Axiom, ClassExpression, ObjectPropertyExpression, SubClassOfAxiom};
use owl2_reasoner::entities::{Class, Individual, NamedIndividual, ObjectProperty};
...
fn test_object_one_of() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let founders_enum = ClassExpression::ObjectOneOf(Box::new(smallvec::smallvec![
        john.clone(),
        mary.clone(),
        bob.clone()
    ]));
