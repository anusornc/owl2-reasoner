...
use owl2_reasoner::{Ontology, TableauxReasoner};
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Direct semantics tests: {}", metadata.direct_semantics_tests);
    ...
    for (i, test) in metadata.sample_tests.iter().enumerate() {
        println!("\n[{}/{}] Running test: {}", i + 1, metadata.sample_tests.len(), test.identifier);
...
fn run_test(test: &TestCase) -> TestResult {
    ...
    let primary_type = test.test_types.first()
        .cloned()
...
fn run_consistency_test(test: &TestCase, expected_consistent: bool) -> (String, Option<bool>, Option<bool>, Option<String>) {
...
fn run_entailment_test(test: &TestCase, expected_entailed: bool) -> (String, Option<bool>, Option<bool>, Option<String>) {
