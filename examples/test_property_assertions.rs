use owl2_reasoner::axioms::{PropertyAssertionAxiom, NegativeObjectPropertyAssertionAxiom, Axiom};
...
fn test_negative_property_assertions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let positive_axiom = PropertyAssertionAxiom::new(
        alice.clone(),
        knows.clone(),
        bob.clone(),
    );
    ...
    let _ = ontology.add_axiom(Axiom::NegativeObjectPropertyAssertion(Box::new(negative_axiom)));
