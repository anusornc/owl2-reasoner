...
fn main() -> OwlResult<()> {
    ...
    println!(
        "✓ Created ontology with {} entities in {:?}",
        large_ontology.entity_count(),
        creation_time
    );
    println!(
        "✓ Creation rate: {:.0} entities/second",
    ...
    println!(
        "✓ Average instance retrieval time: {:?}",
        avg_retrieval_time
    );
    println!(
        "✓ Retrieval rate: {:.0} queries/second",
    ...
    let query_pattern = QueryPattern::BasicGraphPattern(vec![TriplePattern {
    ...
    println!(
        "✓ Property characteristics reasoning completed in {:?}",
        prop_time
    );
...
fn create_large_ontology(size: usize) -> OwlResult<Ontology> {
    ...
    for i in 0..(size / 10) {
        ...
        let assertion =
            ClassAssertionAxiom::new(individual.iri().clone(), ClassExpression::Class(class));
    ...
    for i in 0..(size / 20) {
        ...
        let prop =
            ObjectProperty::new(format!("http://example.org/hasRelation{}", i % (size / 10)));
...
fn analyze_ontology_memory(ontology: &Ontology) -> HashMap<String, usize> {
    ...
    stats.insert(
        ...
        ontology.object_properties().len() * 64,
    );
    stats.insert(
        "named_individuals".to_string(),
        ontology.named_individuals().len() * 32,
    );
    stats.insert("axioms".to_string(), ontology.axioms().len() * 128);
    stats.insert(
        "subclass_axioms".to_string(),
        ontology.subclass_axioms().len() * 64,
    );
    stats.insert(
        "class_assertions".to_string(),
        ontology.class_assertions().len() * 48,
    );
    stats.insert(
        "property_assertions".to_string(),
        ontology.property_assertions().len() * 80,
    );
