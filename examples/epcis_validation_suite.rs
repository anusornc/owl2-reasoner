...
fn main() -> OwlResult<()> {
    ...
    println!("- Comprehensive Standards: {}", validation_config.test_comprehensive_standards);
    println!("- Reasoning Capabilities: {}", validation_config.test_reasoning_capabilities);
    println!("- Performance Requirements: {}", validation_config.test_performance_requirements);
    println!("- Interoperability: {}", validation_config.test_interoperability);
...
fn validate_standards_compliance(events: &[EPCISEvent]) -> OwlResult<StandardsCompliance> {
    ...
    println!("   GS1 EPCIS 2.0 Compliance: {:.1}%", gs1_compliance * 100.0);
    
    // Test data model compliance
    let data_model_compliance = validate_data_model(events)?;
    println!("   Data Model Compliance: {:.1}%", data_model_compliance * 100.0);
    
    // Test event structure compliance
    let event_structure_compliance = validate_event_structure(events)?;
    println!("   Event Structure Compliance: {:.1}%", event_structure_compliance * 100.0);
    
    // Test master data compliance
    let master_data_compliance = validate_master_data(events)?;
    println!("   Master Data Compliance: {:.1}%", master_data_compliance * 100.0);
    
    // Test vocabulary compliance
    let vocabulary_compliance = validate_vocabulary(events)?;
    println!("   Vocabulary Compliance: {:.1}%", vocabulary_compliance * 100.0);
    
    let overall_compliance = (
        gs1_compliance + data_model_compliance + event_structure_compliance + 
        master_data_compliance + vocabulary_compliance
    ) / 5.0;
...
fn validate_gs1_epcis_2_0(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    for event in events {
        ...
        let has_event_type = matches!(event.event_type, 
        ...
        let event_checks = vec![has_event_id, has_event_time, has_event_type, has_epcs, has_action];
        compliant_events += event_checks.iter().filter(|&&x| x).count();
        total_checks += event_checks.len();
    }
    
    Ok(if total_checks > 0 { compliant_events as f64 / total_checks as f64 } else { 1.0 })
}
...
fn validate_data_model(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    Ok(if events.is_empty() { 1.0 } else { compliant_events as f64 / events.len() as f64 })
}
...
fn validate_event_structure(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    for event in events {
        let has_valid_structure = match event.event_type {
            EPCISEventType::AggregationEvent => {
                event.child_epcs.as_ref().map_or(false, |children| !children.is_empty())
            },
            EPCISEventType::ObjectEvent => {
                !event.epc_list.is_empty()
            },
...
fn validate_master_data(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    for event in events {
        let has_master_data = event.read_point.is_some() && 
                              event.business_location.is_some();
    ...
    Ok(if events.is_empty() { 1.0 } else { events_with_master_data as f64 / events.len() as f64 })
}
...
fn validate_reasoning_capabilities(ontology: &Ontology, events: &[EPCISEvent]) -> OwlResult<ReasoningCapabilities> {
    ...
    println!("   Consistency Checking: {:.1}%", consistency_checking * 100.0);
    
    // Test classification reasoning
    let classification_reasoning = validate_classification_reasoning(&reasoner)?;
    println!("   Classification Reasoning: {:.1}%", classification_reasoning * 100.0);
    
    // Test traceability reasoning
    let traceability_reasoning = validate_traceability_reasoning(events)?;
    println!("   Traceability Reasoning: {:.1}%", traceability_reasoning * 100.0);
    
    // Test complex query support
    let complex_query_support = validate_complex_query_support(&reasoner)?;
    println!("   Complex Query Support: {:.1}%", complex_query_support * 100.0);
    
    // Test rule-based reasoning
    let rule_based_reasoning = validate_rule_based_reasoning(&reasoner)?;
    println!("   Rule-based Reasoning: {:.1}%", rule_based_reasoning * 100.0);
    
    let overall_reasoning = (
        consistency_checking + classification_reasoning + traceability_reasoning + 
        complex_query_support + rule_based_reasoning
    ) / 5.0;
...
fn validate_consistency_checking(reasoner: &SimpleReasoner) -> OwlResult<f64> {
    ...
    let performance_score = if response_time < std::time::Duration::from_millis(100) { 1.0 } else { 0.5 };
...
fn validate_classification_reasoning(reasoner: &SimpleReasoner) -> OwlResult<f64> {
    ...
    let performance_score = if response_time < std::time::Duration::from_millis(50) { 1.0 } else { 0.5 };
...
fn validate_traceability_reasoning(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    Ok(if sample_epcs.is_empty() { 1.0 } else { traceable_epcs as f64 / sample_epcs.len() as f64 })
}
...
fn validate_performance_requirements(events: &[EPCISEvent]) -> OwlResult<PerformanceValidation> {
    ...
    println!("   Throughput Requirements: {:.1}%", throughput_requirements * 100.0);
    
    // Test latency requirements
    let latency_requirements = validate_latency(events)?;
    println!("   Latency Requirements: {:.1}%", latency_requirements * 100.0);
    
    // Test scalability requirements
    let scalability_requirements = validate_scalability(events)?;
    println!("   Scalability Requirements: {:.1}%", scalability_requirements * 100.0);
    ...
    let overall_performance = (
        throughput_requirements + latency_requirements + scalability_requirements + memory_efficiency
    ) / 4.0;
...
fn validate_scalability(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    let avg_time = processing_times.iter().sum::<std::time::Duration>() / processing_times.len() as u32;
    let max_time = *processing_times.iter().max().unwrap_or(&std::time::Duration::new(0, 0));
...
fn validate_memory_efficiency(events: &[EPCISEvent]) -> OwlResult<f64> {
    ...
    Ok(if actual_memory_per_event <= target_memory_per_event { 1.0 } else { 0.8 })
}
...
fn validate_interoperability(events: &[EPCISEvent]) -> OwlResult<InteroperabilityValidation> {
    ...
    println!("   Web Service Compatibility: {:.1}%", web_service_compatibility * 100.0);
    ...
    let overall_interoperability = (
        xml_support + json_support + web_service_compatibility + api_compliance
    ) / 4.0;
...
fn validate_edge_case_handling(events: &[EPCISEvent]) -> OwlResult<EdgeCaseHandling> {
    ...
    println!("   Boundary Conditions: {:.1}%", boundary_conditions * 100.0);
    
    // Test malformed data handling
    let malformed_data_handling = validate_malformed_data_handling(events)?;
    println!("   Malformed Data Handling: {:.1}%", malformed_data_handling * 100.0);
    
    let overall_edge_handling = (
        error_handling + exception_recovery + boundary_conditions + malformed_data_handling
    ) / 4.0;
...
fn generate_validation_report(results: &ValidationResults) {
    ...
    println!("  GS1 EPCIS 2.0: {:.1}%", results.standards_compliance.gs1_epcis_2_0_compliance * 100.0);
    println!("  Data Model: {:.1}%", results.standards_compliance.data_model_compliance * 100.0);
    println!("  Event Structure: {:.1}%", results.standards_compliance.event_structure_compliance * 100.0);
    println!("  Master Data: {:.1}%", results.standards_compliance.master_data_compliance * 100.0);
    println!("  Vocabulary: {:.1}%", results.standards_compliance.vocabulary_compliance * 100.0);
    println!("  Overall Standards: {:.1}%", results.standards_compliance.overall_compliance * 100.0);
    
    println!("\nReasoning Capabilities Results:");
    println!("  Consistency Checking: {:.1}%", results.reasoning_capabilities.consistency_checking * 100.0);
    println!("  Classification: {:.1}%", results.reasoning_capabilities.classification_reasoning * 100.0);
    println!("  Traceability: {:.1}%", results.reasoning_capabilities.traceability_reasoning * 100.0);
    println!("  Complex Queries: {:.1}%", results.reasoning_capabilities.complex_query_support * 100.0);
    println!("  Rule-based: {:.1}%", results.reasoning_capabilities.rule_based_reasoning * 100.0);
    println!("  Overall Reasoning: {:.1}%", results.reasoning_capabilities.overall_reasoning * 100.0);
    
    println!("\nPerformance Validation Results:");
    println!("  Throughput: {:.1}%", results.performance_validation.throughput_requirements * 100.0);
    println!("  Latency: {:.1}%", results.performance_validation.latency_requirements * 100.0);
    println!("  Scalability: {:.1}%", results.performance_validation.scalability_requirements * 100.0);
    println!("  Memory Efficiency: {:.1}%", results.performance_validation.memory_efficiency * 100.0);
    println!("  Overall Performance: {:.1}%", results.performance_validation.overall_performance * 100.0);
    
    println!("\nInteroperability Results:");
    println!("  XML Support: {:.1}%", results.interoperability_validation.xml_support * 100.0);
    println!("  JSON Support: {:.1}%", results.interoperability_validation.json_support * 100.0);
    println!("  Web Services: {:.1}%", results.interoperability_validation.web_service_compatibility * 100.0);
    println!("  API Compliance: {:.1}%", results.interoperability_validation.api_compliance * 100.0);
    println!("  Overall Interoperability: {:.1}%", results.interoperability_validation.overall_interoperability * 100.0);
    
    println!("\nEdge Case Handling Results:");
    println!("  Error Handling: {:.1}%", results.edge_case_handling.error_handling * 100.0);
    println!("  Exception Recovery: {:.1}%", results.edge_case_handling.exception_recovery * 100.0);
    println!("  Boundary Conditions: {:.1}%", results.edge_case_handling.boundary_conditions * 100.0);
    println!("  Malformed Data: {:.1}%", results.edge_case_handling.malformed_data_handling * 100.0);
    println!("  Overall Edge Handling: {:.1}%", results.edge_case_handling.overall_edge_handling * 100.0);
    
    println!("\nOverall Validation Score: {:.1}%", results.overall_score * 100.0);
}
